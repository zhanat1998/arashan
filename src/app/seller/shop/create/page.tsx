...
export default function CreateShopPage() {
    ...
    async function checkExistingShop() {
      try {
        ...
        if (res.ok) {
          ...
          if (shop) {
            // User already has a shop, redirect to seller dashboard
            router.push('/seller');
            return;
          }
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        // If shop already exists, redirect to seller dashboard
        if (data.shop) {
          router.push('/seller');
          return;
        }
        throw new Error(data.error || 'Дүкөн түзүүдө ката кетти');
      }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-500 via-red-500 to-pink-500 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-white">Текшерилүүдө...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          <div className="p-6">
            <h1 className="text-2xl font-bold text-gray-800 mb-2 text-center">
              {step === 1 && 'Дүкөн маалыматы'}
              {step === 2 && 'Байланыш маалыматы'}
              {step === 3 && 'Дүкөндү ачуу'}
            </h1>
            <p className="text-gray-500 text-center mb-6">
              {step === 1 && 'Дүкөнүңүздүн негизги маалыматын киргизиңиз'}
              {step === 2 && 'Сатып алуучулар сиз менен байланыша алышы керек'}
              {step === 3 && 'Маалыматты текшерип, дүкөн ачыңыз'}
            </p>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Мисалы: Алтын Дүкөн"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Дүкөнүңүз жөнүндө кыскача маалымат..."
                      rows={3}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    {formData.phone && formData.phone.length < 9 && (
                      <p className="text-xs text-gray-400 mt-1">9 цифра керек ({formData.phone.length}/9)</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {formData.whatsapp && formData.whatsapp.length < 9 && (
                      <p className="text-xs text-gray-400 mt-1">9 цифра керек ({formData.whatsapp.length}/9)</p>
                    )}
                  </div>
              ...
              {step === 3 && (
                <div className="space-y-4">
                  <div className="bg-gray-50 rounded-2xl p-6">
                    <h3 className="font-semibold text-gray-800 mb-4">Дүкөн маалыматы</h3>
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span className="text-gray-500">Аты:</span>
                        <span className="font-medium text-gray-800">{formData.name || '-'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">Жайгашкан жери:</span>
                        <span className="font-medium text-gray-800">{formData.location || '-'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">Телефон:</span>
                        <span className="font-medium text-gray-800">{formData.phone ? `+996 ${formData.phone}` : '-'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">WhatsApp:</span>
                        <span className="font-medium text-gray-800">{formData.whatsapp ? `+996 ${formData.whatsapp}` : '-'}</span>
                      </div>
                      <div className="flex justify-between">
                    ...
                    {formData.description && (
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        <span className="text-gray-500 text-sm">Сүрөттөмө:</span>
                  ...
                  <div className="bg-green-50 rounded-xl p-4 flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium text-green-800">Дүкөн ачууга даярсыз!</p>
                      <p className="text-sm text-green-600 mt-1">
                        Дүкөн ачкандан кийин товарларды кошуп баштай аласыз.
                      </p>
                    </div>
                  </div>
                </div>
